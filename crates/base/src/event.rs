...
static  WINDOW_STATE: RwLock<WindowState> = RwLock::new(WindowState::Resumed);
pub static IS_CHANGED: AtomicBool = AtomicBool::new(false);

#[cfg(feature = "pi_js_export")]
pub fn on_resumed(_engine: &mut Engine, _window: &Arc<Window>) {
    // await_last_frame(engine);
    println!("----------on_resumed222222");
    *WINDOW_STATE.write().unwrap() = WindowState::Resumed;
    let _ = IS_CHANGED.store(true, Ordering::Relaxed);

}

#[cfg(feature = "pi_js_export")]
pub fn on_suspended(_engine: &mut Engine, version: String) {
    if version.contains("Android 13") {}
    println!("----------on_suspended222222: {}", version);
    // let world = &mut engine.app.world;
    *WINDOW_STATE.write().unwrap() = WindowState::Suspended;
...
pub fn on_change(engine: &mut Engine) {
    engine
        ...
        .0
        .take();
    match *WINDOW_STATE.read().unwrap() {
        WindowState::Resumed => {
