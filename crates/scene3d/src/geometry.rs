...
impl VBMeta {
    #[cfg_attr(target_arch="wasm32", wasm_bindgen)]
    #[cfg(feature = "pi_js_export")]
    pub fn create(name: pi_export_base::export::Atom, start: Option<f64>, end: Option<f64>) -> Self {
    ...
    // #[cfg_attr(target_arch="wasm32", wasm_bindgen)]
    // #[cfg(feature = "pi_js_export")]
    // pub fn create_as_instance(name: pi_export_base::export::Atom, start: Option<f64>, end: Option<f64>) -> Self {
    //     let range = if let (Some(start), Some(end)) = (start, end) {
    //         Some(Range { start: start as u64, end: end as u64 })
    //     } else {
    //         None
...
#[cfg_attr(target_arch="wasm32", wasm_bindgen)]
#[cfg(feature = "pi_js_export")]
pub struct IBMeta(pub(crate) IndicesBufferDesc);
#[cfg_attr(target_arch="wasm32", wasm_bindgen)]
#[cfg(feature = "pi_js_export")]
impl IBMeta {
    #[cfg_attr(target_arch="wasm32", wasm_bindgen)]
    #[cfg(feature = "pi_js_export")]
    pub fn create(name: pi_export_base::export::Atom, start: Option<f64>, end: Option<f64>, as_u16: bool) -> Self {
        let range = if let (Some(start), Some(end)) = (start, end) {
            Some(Range { start: start as u64, end: end as u64 })
        } else {
            None
...
#[cfg_attr(target_arch="wasm32", wasm_bindgen)]
#[cfg(feature = "pi_js_export")]
pub fn p3d_geo_set_indice(geo: &mut GeometryMeta, ib: IBMeta) {
    geo.1 = Some(ib.0);
}
#[cfg_attr(target_arch="wasm32", wasm_bindgen)]
#[cfg(feature = "pi_js_export")]
